...
async function getBookings(filters: SearchParams) {
  ...
  let query = supabase
    ...
    .select("id, candidate_id, scheduled_at, end_at, meeting_link, status, title, attendee_name, attendee_email, attendee_phone, reschedule_link, cancellation_link, notes")
    ...
    <div className="space-y-6">
      <div>
        <h1 className="flex items-center gap-2 font-display text-2xl">
          ...
          <HelpTooltip text="Les réservations Cal.com apparaissent automatiquement ici. Vous pouvez rechercher un contact, filtrer les rendez-vous et mettre à jour leur suivi interne." />
        </h1>
      ...
      <form className="grid gap-3 rounded-2xl border border-border/70 bg-card p-4 sm:grid-cols-[1fr_180px_180px_auto]">
        ...
        <div className="overflow-x-auto rounded-2xl border border-border/70 bg-card">
          <table className="w-full min-w-[1050px] text-sm">
            <thead className="border-b border-border bg-secondary/40 text-left">
              <tr><th className="p-4">Date</th><th className="p-4">Contact</th><th className="p-4">Rendez-vous</th><th className="p-4">Statut</th><th className="p-4">Actions</th></tr>
            </thead>
            <tbody>
